...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/07 17:25:16 by ldemesla          #+#    #+#             */
/*   Updated: 2019/11/07 20:26:05 by ldemesla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			write_rgb(t_data *data, int height, int width, int fd)
{
	unsigned char	abc[3];
	int				i_j[2];
	...
	while (i_j[0] < height)
	{
		...
		while (i_j[1] < width)
		{
			abc[0] = (240 & (data->img.data[(height - i_j[0]) *
			width + i_j[1]]) >> 16);
			abc[1] = (240 & (data->img.data[(height - i_j[0]) *
			width + i_j[1]]) >> 8);
			abc[2] = (240 & data->img.data[(height - i_j[0]) *
			width + i_j[1]]);
			write(fd, abc + 2, 1);
			write(fd, abc + 1, 1);
			write(fd, abc, 1);
...
void			generate_bitmap_image(t_data *data, int height, int width)
{
	int				fd;
	...
	write_rgb(data, height, width, fd);
	close(fd);
}
...
int				create_bmp(t_data *data)
{
	...
	height = data->height;
	width = data->width;
	generate_bitmap_image(data, height, width);
	return (1);
}
